    ...
    async def list_for_room(
        ...
        *,
        ...
        if room_id is not None:
            ...
            count_stmt = count_stmt.where(Tag.room_id == room_id)
        if platform is not None:
            stmt = stmt.where(Tag.platform == platform)
            count_stmt = count_stmt.where(Tag.platform == platform)
        if active_only:
            stmt = stmt.where(Tag.is_active.is_(True))
            count_stmt = count_stmt.where(Tag.is_active.is_(True))
        stmt = stmt.order_by(Tag.value.asc())
